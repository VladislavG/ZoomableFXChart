...
public class DateAxis310 extends Axis<LocalDateTime> {
    ...
    @Override protected List<LocalDateTime> calculateTickValues(final double VALUE, final Object RANGE) {
        ...
        List<LocalDateTime> evenDateList = makeDatesEven(dateList, calendar);
        ...
        if (evenDateList.size() > 2) {
            ...
            if (secondDate.toEpochSecond(ZoneOffset.ofHours(0)) - lower.toEpochSecond(ZoneOffset.ofHours(0)) < thirdDate.toEpochSecond(ZoneOffset.ofHours(0)) - secondDate.toEpochSecond(ZoneOffset.ofHours(0))) {
    ...
    private List<LocalDateTime> makeDatesEven(List<LocalDateTime> dates, LocalDateTime calendar) {
            ...
            for (int i = 0; i < dates.size(); i++) {
                ...
                switch (actualInterval.INTERVAL) {
                    ...
                    case WEEKS:
                        ...
                        calendar.withMinute(0);
                        calendar.withSecond(0);
                        calendar.withNano(4000000);
                        break;
                    case DAYS:
                        calendar.withHour(0);
                        calendar.withMinute(0);
                        calendar.withSecond(0);
                        calendar.withNano(3000000);
                        break;
                    case HOURS:
                        if (i != 0 && i != dates.size() - 1) {
                            calendar.withMinute(0);
                        ...
                        calendar.withNano(2000000);
                        break;
                    case MINUTES:
                        if (i != 0 && i != dates.size() - 1) {
                            calendar.withSecond(0);
                        }
                        calendar.withNano(1000000);
                        break;
                    case SECONDS:
                        calendar.withSecond(0);
                        break;
    ...
    private long toMillis(final LocalDateTime DATE_TIME) {
        return DATE_TIME.toInstant(ZoneOffset.ofHours(0)).getEpochSecond() * 1000l;
    }
